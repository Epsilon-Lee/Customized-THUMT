...
import thumt.data.dataset_c2f_4layers_sc as dataset_c2f_4layers_sc
...
def parse_args(args=None):
    ...
    parser.add_argument("--index-map", type=str, nargs=4,
                        help="Paths of index mapping between two label (vocab) set over the hierarchy")
...
def override_parameters(params, args):
    ...
    params.index_map = args.index_map or params.index_map
...
def main(args):
    ...
    with tf.Graph().as_default():
        if not params.record:
            # Build input queue
            features, constant_features = dataset_c2f_4layers_sc.get_training_input_and_c2f_label(
                params.input,
                params.c2f_input,
                params.index_map,
                params
        ...
        #features, init_op = cache.cache_features_with_constant(features, constant_features, update_cycle)
        ...
        if len(sharded_losses) > 1:
            losses_mle, losses_l1, losses_l2, losses_l3, losses_l4, kl_total_losses = sharded_losses
            ...
            kl_total_loss = tf.add_n(kl_total_losses) / len(kl_total_losses)
        else:
            loss_mle, loss_l1, loss_l2, loss_l3, loss_l4, kl_total_loss = sharded_losses[0]
        loss = loss_mle + (loss_l1 + loss_l2 + loss_l3 + loss_l4)/4.0 + kl_total_loss + tf.losses.get_regularization_loss()
        ...
        if params.validation and params.references[0]:
            ...
            eval_inputs = dataset_c2f_4layers_sc.sort_and_zip_files(files)
            eval_input_fn = dataset_c2f_4layers_sc.get_evaluation_input
        else:
            eval_input_fn = None
