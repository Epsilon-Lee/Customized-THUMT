...
def get_training_input_and_c2f_label(filenames, c2f_filenames, regemb_filenames, params):
    ...
    with tf.device("/cpu:0"):
        ...
        tgt_l0_dataset = tf.data.TextLineDataset(c2f_filenames[0])

        l0_dict_dataset = tf.data.TextLineDataset(regemb_filenames[0])
        word_dict_dataset = tf.data.TextLineDataset(regemb_filenames[1])
        l0_word_dict_dataset = tf.data.Dataset.zip((l0_dict_dataset, word_dict_dataset))
        l0_word_dict_dataset = l0_word_dict_dataset.shuffle(params.buffer_size)
        l0_word_dict_dataset = l0_word_dict_dataset.repeat()

        dataset = tf.data.Dataset.zip((src_dataset, tgt_dataset, tgt_l0_dataset))
        dataset = dataset.shuffle(params.buffer_size)
        dataset = dataset.repeat()

        # Split string
        dataset = dataset.map(
            lambda src, tgt, tgt_l0: (
                tf.string_split([src]).values,
                tf.string_split([tgt]).values,
                tf.string_split([tgt_l0]).values,
            ),
            num_parallel_calls=params.num_threads
        )

        l0_word_dict_dataset = l0_word_dict_dataset.map(
            lambda l0, word: (
                tf.string_split([l0]).values,
                tf.string_split([word]).values,
            ),
            num_parallel_calls=params.num_threads
        )
        ...
        dataset = dataset.map(
            lambda src, tgt, tgt_l0: (
                ...
                tf.concat([tgt_l0, [tf.constant(params.eos)]], axis=0),
            ),
            num_parallel_calls=params.num_threads
        )
        ...
        l0_word_dict_dataset = l0_word_dict_dataset.map(
            lambda l0, word: {
                "l0": l0,
                "word": word,
                "l0_length": tf.shape(l0),
                "word_length": tf.shape(word),
            },
            num_parallel_calls=params.num_threads
        )
        ...
        iterator_4dict = l0_word_dict_dataset.make_one_shot_iterator()
        features_4dict = iterator_4dict.get_next()
        ...
        tgt_l0_table = tf.contrib.lookup.index_table_from_tensor(
            tf.constant(params.vocabulary["target_l0"]),
            default_value=params.mapping["target_l0"][params.unk]
        )

        # String to index lookup
        features["source"] = src_table.lookup(features["source"])
        features["target"] = tgt_table.lookup(features["target"])
        features["target_l0"] = tgt_l0_table.lookup(features["target_l0"])
        
        features_4dict["l0"] = tgt_l0_table.lookup(features_4dict["l0"])
        features_4dict["word"] = tgt_table.lookup(features_4dict["word"])
        ...
        shard_multiplier = len(params.device_list) * params.update_cycle
        features_4dict = batch_examples(features_4dict, params.batch_size,
                                  params.max_length, params.mantissa_bits,
                                  shard_multiplier=shard_multiplier,
                                  length_multiplier=params.length_multiplier,
                                  constant=params.constant_batch_size,
        ...
        features["target_l0"] = tf.to_int32(features["target_l0"])
        ...
        features["l0"] = tf.to_int32(features_4dict["l0"])
        features["word"] = tf.to_int32(features_4dict["word"])
        features["l0_length"] = tf.to_int32(features_4dict["l0_length"])
        features["word_length"] = tf.to_int32(features_4dict["word_length"])
        features["l0_length"] = tf.squeeze(features_4dict["l0_length"], 1)
        features["word_length"] = tf.squeeze(features_4dict["word_length"], 1)
