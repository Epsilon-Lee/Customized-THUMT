...
def multinomial_kl_div(tensor_left, tensor_right):
    """ Compute KL divergence between two multinomial probability vector """
    
    log_left = tf.log(tensor_left)
    log_right = tf.log(tensor_right)
    log_diff = log_left - log_right
    kl = tf.reduce_sum(tensor_left * log_diff, axis=-1)
