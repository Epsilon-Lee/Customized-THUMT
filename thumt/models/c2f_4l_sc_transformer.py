...
def decoding_graph_4layers_sc(features, state, mode, params):
    if mode != "train":
        params.residual_dropout = 0.0
        params.attention_dropout = 0.0
        params.relu_dropout = 0.0
    ...
    tgt_vocab_size = len(tvocab)
    if mode == "train":
        t_l1_vocab = params.vocabulary["target_l1"]
        tgt_l1_vocab_size = len(t_l1_vocab)
        t_l2_vocab = params.vocabulary["target_l2"]
        tgt_l2_vocab_size = len(t_l2_vocab)
        t_l3_vocab = params.vocabulary["target_l3"]
        tgt_l3_vocab_size = len(t_l3_vocab)
        t_l4_vocab = params.vocabulary["target_l4"]
    ...
    if params.shared_embedding_and_softmax_weights:
        ...
        weights = tf.get_variable("softmax", [tgt_vocab_size, hidden_size],
                                  initializer=initializer)
        if mode == "train":
            weights_l1 = tf.get_variable("softmax_l1", [tgt_l1_vocab_size, hidden_size],
                                    initializer=initializer)
            weights_l2 = tf.get_variable("softmax_l2", [tgt_l2_vocab_size, hidden_size],
                                    initializer=initializer)
            weights_l3 = tf.get_variable("softmax_l3", [tgt_l3_vocab_size, hidden_size],
                                    initializer=initializer)
    ...
    logits = tf.matmul(decoder_output, weights, False, True)
    logits_l1 = tf.matmul(decoder_output_l1, weights_l1, False, True)
    logits_l2 = tf.matmul(decoder_output_l2, weights_l2, False, True)
    logits_l3 = tf.matmul(decoder_output_l3, weights_l3, False, True)
    logits_l4 = tf.matmul(decoder_output_l4, weights_l4, False, True)

    softmax = tf.nn.softmax(logits)
    softmax_l1 = tf.nn.softmax(logits_l1)
    softmax_l2 = tf.nn.softmax(logits_l2)
    softmax_l3 = tf.nn.softmax(logits_l3)
    softmax_l4 = tf.nn.softmax(logits_l4)

    softmax_t = tf.transpose(softmax)
    softmax_l1_t = tf.transpose(softmax_l1)  # [V1, ...]
    softmax_l2_t = tf.transpose(softmax_l2)  # [V2, ...]
    softmax_l3_t = tf.transpose(softmax_l3)  # [V3, ...]
    softmax_l4_t = tf.transpose(softmax_l4)  # [V4, ...]
    
    index_map_l1 = features["index_map_l1"]
    index_map_l2 = features["index_map_l2"]
    index_map_l3 = features["index_map_l3"]
    index_map_l4 = features["index_map_l4"]

    index_map_l1_mask = tf.expand_dims(features["index_map_l1_mask"], axis=-1)
    index_map_l2_mask = tf.expand_dims(features["index_map_l2_mask"], axis=-1)
    index_map_l3_mask = tf.expand_dims(features["index_map_l3_mask"], axis=-1)
    index_map_l4_mask = tf.expand_dims(features["index_map_l4_mask"], axis=-1)
    #index_map_l1_mask = features["index_map_l1_mask"]
    #index_map_l2_mask = features["index_map_l2_mask"]
    #index_map_l3_mask = features["index_map_l3_mask"]
    #index_map_l4_mask = features["index_map_l4_mask"]

    # In this way, OOM error
    #import ipdb; ipdb.set_trace()
    softmax_l2_t_proj = utils.common.assembly_gather_reduce_sum(softmax_l2_t, index_map_l1, index_map_l1_mask)
    softmax_l3_t_proj = utils.common.assembly_gather_reduce_sum(softmax_l3_t, index_map_l2, index_map_l2_mask)
    softmax_l4_t_proj = utils.common.assembly_gather_reduce_sum(softmax_l4_t, index_map_l3, index_map_l3_mask)
    softmax_t_proj = utils.common.assembly_gather_reduce_sum(softmax_t, index_map_l4, index_map_l4_mask)

    #softmax_l2_t_proj = tf.reduce_sum(softmax_l2_t_proj * index_map_l1_mask, axis=1)
    #softmax_l3_t_proj = tf.reduce_sum(softmax_l3_t_proj * index_map_l2_mask, axis=1)
    #softmax_l4_t_proj = tf.reduce_sum(softmax_l4_t_proj * index_map_l3_mask, axis=1)
    #softmax_t_proj = tf.reduce_sum(softmax_t_proj * index_map_l4_mask, axis=1)

    softmax_l2_proj = tf.transpose(softmax_l2_t_proj)
    softmax_l3_proj = tf.transpose(softmax_l3_t_proj)
    softmax_l4_proj = tf.transpose(softmax_l4_t_proj)
    softmax_proj = tf.transpose(softmax_t_proj)

    kl_l2_l1 = layers.nn.multinomial_kl_div(softmax_l2_proj, softmax_l1)
    kl_l3_l2 = layers.nn.multinomial_kl_div(softmax_l3_proj, softmax_l2)
    kl_l4_l3 = layers.nn.multinomial_kl_div(softmax_l4_proj, softmax_l3)
    kl_final_l4 = layers.nn.multinomial_kl_div(softmax_proj, softmax_l4)

    labels = features["target"]
    labels_l1 = features["target_l1"]
    labels_l2 = features["target_l2"]
    labels_l3 = features["target_l3"]
    labels_l4 = features["target_l4"]
    ...
    kl_l2_l1 = tf.reshape(kl_l2_l1, tf.shape(tgt_seq))
    kl_l3_l2 = tf.reshape(kl_l3_l2, tf.shape(tgt_seq))
    kl_l4_l3 = tf.reshape(kl_l4_l3, tf.shape(tgt_seq))
    kl_final_l4 = tf.reshape(kl_final_l4, tf.shape(tgt_seq))
    ...
    kl_l2_l1 = tf.reduce_sum(kl_l2_l1 * tgt_mask) / tf.reduce_sum(tgt_mask)
    kl_l3_l2 = tf.reduce_sum(kl_l3_l2 * tgt_mask) / tf.reduce_sum(tgt_mask)
    kl_l4_l3 = tf.reduce_sum(kl_l4_l3 * tgt_mask) / tf.reduce_sum(tgt_mask)
    kl_final_l4 = tf.reduce_sum(kl_final_l4 * tgt_mask) / tf.reduce_sum(tgt_mask)
    kl_total_loss = 1.0/4.0 * (kl_l2_l1 + kl_l3_l2 + kl_l4_l3 + kl_final_l4)
    ...
    tf.summary.scalar("kl_l2_l1", kl_l2_l1)
    tf.summary.scalar("kl_l3_l2", kl_l3_l2)
    tf.summary.scalar("kl_l4_l3", kl_l4_l3)
    tf.summary.scalar("kl_final_l4", kl_final_l4)
    tf.summary.scalar("loss_l1", loss_l1)
    tf.summary.scalar("loss_l2", loss_l2)
    tf.summary.scalar("loss_l3", loss_l3)
    tf.summary.scalar("loss_l4", loss_l4)
    tf.summary.scalar("loss", loss)

    return loss, loss_l1, loss_l2, loss_l3, loss_l4, kl_total_loss


def decoding_graph_4layers_with_task_uncertainty(features, state, mode, params):
    if mode != "train":
        params.residual_dropout = 0.0
        params.attention_dropout = 0.0
        params.relu_dropout = 0.0
    ...
    s_l5 = tf.get_variable("uncertainty_l5", initializer=tf.constant(5.0))
    s_l4 = tf.get_variable("uncertainty_l4", initializer=tf.constant(5.0))
    s_l3 = tf.get_variable("uncertainty_l3", initializer=tf.constant(5.0))
    s_l2 = tf.get_variable("uncertainty_l2", initializer=tf.constant(5.0))
    s_l1 = tf.get_variable("uncertainty_l1", initializer=tf.constant(5.0))
    ...
    tf.summary.scalar("loss_l5", loss)
    tf.summary.scalar("loss_l1", loss_l1)
    tf.summary.scalar("loss_l2", loss_l2)
    tf.summary.scalar("loss_l3", loss_l3)
    tf.summary.scalar("loss_l4", loss_l4)
    tf.summary.scalar("sigma_l5", tf.exp(s_l5))
    tf.summary.scalar("sigma_l1", tf.exp(s_l1))
    tf.summary.scalar("sigma_l2", tf.exp(s_l2))
    tf.summary.scalar("sigma_l3", tf.exp(s_l3))
    tf.summary.scalar("sigma_l4", tf.exp(s_l4))

    return loss * tf.exp(-s_l5) + s_l5 / 2.0, loss_l1 * tf.exp(-s_l1) + s_l1 / 2.0, loss_l2 * tf.exp(-s_l2) + s_l2 / 2.0, loss_l3 * tf.exp(-s_l3) + s_l3 / 2.0, loss_l4 * tf.exp(-s_l4) + s_l4 / 2.0
...
def uncertainty_weighed_layer_fusion_decoding_graph(features, state, mode, params):
    ...
    ent = tf.stack([ent_l5, ent_l4, ent_l3, ent_l2, ent_l1, ent_l0], axis=1)  # [N x T, 6]
    ent_norm = tf.nn.softmax(ent, -1)  # [N x T, 6]
    
    coef_l5, coef_l4, coef_l3, coef_l2, coef_l1, coef_l0 = tf.split(ent_norm, 6, axis=1)  # [N x T, 1]
    logits_l5_weighed = coef_l5 * logits_l5
    logits_l4_weighed = coef_l4 * logits_l4
    logits_l3_weighed = coef_l3 * logits_l3
    logits_l2_weighed = coef_l2 * logits_l2
    logits_l1_weighed = coef_l1 * logits_l1
    logits_l0_weighed = coef_l0 * logits_l0

    fused_logits = logits_5_weighed + logits_4_weighed + logits_3_weighed + logits_2_weighed + logits_1_weighed + logits_0_weighed
...
def model_graph(features, mode, params):
    ...
    if params.submodel == "c2f-4l-sc":
        print("Submodel Type: %s" % params.submodel)
        output = decoding_graph_4layers_sc(features, state, mode, params)
    elif params.submodel == "c2f-4l-uncertain":
        print("Submodel Type: %s" % params.submodel)
        output = decoding_graph_4layers_with_task_uncertainty(features, state, mode, params)
...
class Transformer(interface.NMTModel):
    ...
    def get_inference_func(self):
        ...
        def decoding_fn(features, state, params=None):
            ...
            with tf.variable_scope(self._scope):
                if params.submodel == "c2f-4l":
                    log_prob, new_state = decoding_graph_4layers(features, state, "infer",
                                                                params)
                elif params.submodel == "c2f-4l-uncertain":
                    log_prob, new_state = decoding_graph_4layers_with_task_uncertainty(features, state, "infer",
                                                                   params)
