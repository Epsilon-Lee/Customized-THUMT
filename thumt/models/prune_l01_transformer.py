...
def decoding_graph_prune_l1_v0(features, state, mode, params):
    if mode != "train":
        params.residual_dropout = 0.0
        params.attention_dropout = 0.0
        params.relu_dropout = 0.0
    ...
    tvocab_l0 = params.vocabulary["target_l0"]
    tvocab_l1 = params.vocabulary["target_l1"]
    tgt_vocab_size = len(tvocab)
    tgt_l0_vocab_size = len(tvocab_l0)
    tgt_l1_vocab_size = len(tvocab_l1)
    initializer = tf.random_normal_initializer(0.0, params.hidden_size ** -0.5)
    ...
    if params.shared_embedding_and_softmax_weights:
        ...
        weights = tf.get_variable("softmax", [tgt_vocab_size, hidden_size],
                                  initializer=initializer)
        weights_l0 = tf.get_variable("weights_l0", [tgt_l0_vocab_size, hidden_size],
                                    initializer=initializer)
        weights_l1 = tf.get_variable("weights_l1", [tgt_l1_vocab_size, hidden_size],
                                    initializer=initializer)
    ...
    if mode != "infer":
        ...
        l0_logits = tf.matmul(decoder_output, weights_l0, False, True)  # [N, V_L0]
        l0_softmax = tf.nn.softmax(l0_logits)  # [N, V_L0]
        expected_l0_emb = tf.matmul(l0_softmax, weights_l0)  # [N, D]
        # Residual
        decoder_output = _residual_fn(decoder_output, expected_l0_emb)
        
        l1_logits = tf.matmul(decoder_output, weights_l1, False, True)  # [N, V_L1]
        l1_softmax = tf.nn.softmax(l1_logits)
        expected_l1_emb = tf.matmul(l1_softmax, weights_l1)  # [N, D]
        decoder_output = _residual_fn(decoder_output, expected_l1_emb)
    ...
    # Coarse-to-fine modification of decoder_output
    # layer 0
    l0_logits = tf.matmul(decoder_output, weights_l0, False, True)  # [N, V_L0]
    l0_softmax = tf.nn.softmax(l0_logits)  # [N, V_L0]
    expected_l0_emb = tf.matmul(l0_softmax, weights_l0)  # [N, D]
    decoder_output = _residual_fn(decoder_output, expected_l0_emb, params.residual_dropout)
    # layer 1
    l1_logits = tf.matmul(decoder_output, weights_l1, False, True)
    l1_softmax = tf.nn.softmax(l1_logits)
    expected_l1_emb = tf.matmul(l1_softmax, weights_l1)
    decoder_output = _residual_fn(decoder_output, expected_l1_emb, params.residual_dropout)
    ...
    return loss, loss_l0, loss_l1


def decoding_graph_prune_l1_v0_ln(features, state, mode, params):
    if mode != "train":
        params.residual_dropout = 0.0
        params.attention_dropout = 0.0
        params.relu_dropout = 0.0
    ...
    if mode != "infer":
        ...
        l0_softmax = tf.nn.softmax(l0_logits)  # [N, V_L0]
        expected_l0_emb = tf.matmul(l0_softmax, weights_l0)  # [N, D]
        expected_l0_emb = _layer_process(expected_l0_emb, params.layer_postprocess)
        ...
        l1_softmax = tf.nn.softmax(l1_logits)
        expected_l1_emb = tf.matmul(l1_softmax, weights_l1)  # [N, D]
        expected_l1_emb = _layer_process(expected_l1_emb, params.layer_postprocess)
        decoder_output = _residual_fn(decoder_output, expected_l1_emb)
...
def decoding_graph_prune_l1_v1(features, state, mode, params):
    if mode != "train":
        params.residual_dropout = 0.0
        params.attention_dropout = 0.0
        params.relu_dropout = 0.0
    ...
    if mode != "infer":
        ...
        expected_l0_emb = tf.matmul(l0_softmax, weights_l0)  # [N, D]
        # Direct add
        decoder_output = decoder_output +  expected_l0_emb
        
        l1_logits = tf.matmul(decoder_output, weights_l1, False, True)  # [N, V_L1]
        l1_softmax = tf.nn.softmax(l1_logits)
        expected_l1_emb = tf.matmul(l1_softmax, weights_l1)  # [N, D]
        decoder_output = decoder_output + expected_l1_emb
...
def decoding_graph_prune_l1_v1_ln(features, state, mode, params):
    if mode != "train":
        params.residual_dropout = 0.0
        params.attention_dropout = 0.0
        params.relu_dropout = 0.0
...
def decoding_graph_prune_l1_v2(features, state, mode, params):
    if mode != "train":
        params.residual_dropout = 0.0
        params.attention_dropout = 0.0
        params.relu_dropout = 0.0
...
def model_graph(features, mode, params):
    ...
    print("Sub Model Type: %s" % params.submodel)
    #import ipdb; ipdb.set_trace()
    if params.submodel == "prune-l1-v0":
        output = decoding_graph_prune_l1_v0(features, state, mode, params)
    elif params.submodel == "prune-l1-v0-ln":
        output = decoding_graph_prune_l1_v0_ln(features, state, mode, params)
    elif params.submodel == "prune-l1-v1":
        output = decoding_graph_prune_l1_v1(features, state, mode, params)
    elif params.submodel == "prune-l1-v1-ln":
        output = decoding_graph_prune_l1_v1_ln(features, state, mode, params)
    elif params.submodel == "prune-l1-v2":
        output = decoding_graph_prune_l1_v2(features, state, mode, params)
    else:
        raise ValueError("params.submodel should be in submodel types!")
...
class Transformer(interface.NMTModel):
    ...
    def get_inference_func(self):
        ...
        def decoding_fn(features, state, params=None):
            ...
            with tf.variable_scope(self._scope):
                print("Building inference graph with Sub Model Type %s" % params.submodel)
                if params.submodel == "prune-l1-v0":
                    log_prob, new_state = decoding_graph_prune_l1_v0(features, state, "infer",
                                                                    params)
                elif params.submodel == "prune-l1-v0-ln":
                    log_prob, new_state = decoding_graph_prune_l1_v0_ln(features, state, "infer",
                                                                    params)
                elif params.submodel == "prune-l1-v1":
                    log_prob, new_state = decoding_graph_prune_l1_v1(features, state, "infer",
                                                                    params)
                elif params.submodel == "prune-l1-v1-ln":
                    log_prob, new_state = decoding_graph_prune_l1_v1_ln(features, state, "infer",
                                                                    params)
                elif params.submodel == "prune-l1-v2":
                    log_prob, new_state = decoding_graph_prune_l1_v2(features, state, "infer",
                                                                    params)
