...
def decoding_graph_prune_l0_v1(features, state, mode, params):
    if mode != "train":
        params.residual_dropout = 0.0
        params.attention_dropout = 0.0
        params.relu_dropout = 0.0
    ...
    if params.shared_embedding_and_softmax_weights:
        ...
        weights_l0 = tf.get_variable("weights_l0", [tgt_l0_vocab_size, hidden_size],
                                    initializer=initializer)
        gate_weights_l0 = tf.get_variable("gate_weights_l0", [hidden_size, hidden_size],
                                        initializer=initializer)
    ...
    if mode != "infer":
        ...
        # Gating mechanism
        pre_gate = tf.matmul(expected_l0_emb, gate_weights_l0)
        filter_gate = tf.sigmoid(pre_gate)
        decoder_output = filter_gate * decoder_output
    ...
    expected_l0_emb = tf.matmul(l0_softmax, weights_l0)  # [N, D]
    # Gating mechanism
    pre_gate = tf.matmul(expected_l0_emb, gate_weights_l0)  # [N, D]
    filter_gate = tf.sigmoid(pre_gate)  # [N, D]
    decoder_output = filter_gate * decoder_output
...
def decoding_graph_prune_l0_v2(features, state, mode, params):
    if mode != "train":
        params.residual_dropout = 0.0
        params.attention_dropout = 0.0
        params.relu_dropout = 0.0
...
def decoding_graph_prune_l0_regemb(features, state, mode, params):
    if mode != "train":
        params.residual_dropout = 0.0
        params.attention_dropout = 0.0
        params.relu_dropout = 0.0
    ...
    if mode == "train":
        l0 = features["l0"]
        word = features["word"]
        word_length = features["word_length"]
        word_mask = tf.sequence_mask(word_length,
                                    maxlen=tf.shape(word)[1],
                                    dtype=tf.float32)
    ...
    targets = tf.gather(tgt_embedding, tgt_seq)
    # Get l0 label embedding and word embedding
    if mode == "train":
        l0_emb = tf.gather(weights_l0, l0)  # [N, D]
        word_emb = tf.gather(weights, word)  # [N, L, D]
        word_emb = tf.expand_dims(word_mask, 2) * word_emb  # [N, L, 1] * [N, L, D]
        sum_emb = tf.reduce_sum(word_emb, 1)  # [N, D]
        mean_emb = sum_emb / tf.reduce_sum(word_mask, 1, keep_dims=True)  # [N, D]
    ...
    mse_emb = tf.norm(l0_emb - mean_emb) / tf.to_float(tf.shape(word)[0])
...
def model_graph(features, mode, params):
    ...
    if params.submodel == "prune-l0-v0":
        output = decoding_graph_prune_l0_v0(features, state, mode, params)
    elif params.submodel == "prune-l0-v1":
        output = decoding_graph_prune_l0_v1(features, state, mode, params)
    elif params.submodel == "prune-l0-v2":
        output = decoding_graph_prune_l0_v2(features, state, mode, params)
    elif params.submodel == "prune-l0-regemb":
        output = decoding_graph_prune_l0_regemb(features, state, mode, params)
    else:
    ...
    def get_inference_func(self):
        ...
        def decoding_fn(features, state, params=None):
            ...
            with tf.variable_scope(self._scope):
                ...
                if params.submodel == "prune-l0-v0":
                    log_prob, new_state = decoding_graph_prune_l0_v0(features, state, "infer",
                                                                    params)
                elif params.submodel == "prune-l0-v1":
                    log_prob, new_state = decoding_graph_prune_l0_v1(features, state, "infer",
                                                                    params)
                elif params.submodel == "prune-l0-v2":
                    log_prob, new_state = decoding_graph_prune_l0_v2(features, state, "infer",
                                                                    params)
                elif params.submodel == "prune-l0-regemb":
                    log_prob, new_state = decoding_graph_prune_l0_regemb(features, state, "infer",
                                                                    params)
