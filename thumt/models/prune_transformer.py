...
def decoding_graph(features, state, mode, params):
    ...
    tvocab_l0 = params.vocabulary["targer_l0"]
    ...
    if params.shared_embedding_and_softmax_weights:
        ...
        weights = tf.get_variable("softmax", [tgt_vocab_size, hidden_size],
                                  initializer=initializer)
        weights_l0 = tf.get_variable("softmax", [tgt_l0_vocab_size, hidden_size)],
                                    initializer=initializer)
    ...
    l0_softmax = tf.nn.softmax(l0_logits)  # [N, V_L0]
    expected_l0_emb = tf.matmul(l0_softmax, weights_l0)  # [N, D]
    decoder_output = _residual_fn(decoder_output, expected_l0_em, params.residual_dropout)
