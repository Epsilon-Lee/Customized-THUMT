...
import thumt.data.dataset_c2f_regemb as dataset_c2f_regemb
...
def parse_args(args=None):
    ...
    parser.add_argument("--c2f-vocabmap", type=str, nargs=2,
                        help="Path of target coarse-to-fine label vocabulary")
...
def override_parameters(params, args):
    ...
    params.c2f_vocab_map = args.c2f_vocabmap or params.c2f_vocab_map
...
def main(args):
    ...
    with tf.Graph().as_default():
        if not params.record:
            # Build input queue
            features = dataset_c2f_regemb.get_training_input_and_c2f_label(params.input, params.c2f_input, params.c2f_vocab_map, params)
        else:
        ...
        if len(sharded_losses) > 1:
            losses_mle, losses_l0, losses_mse = sharded_losses
            loss_mle = tf.add_n(losses_mle) / len(losses_mle)
            loss_l0 = tf.add_n(losses_l0) / len(losses_l0)
            loss_mse = tf.add_n(losses_mse) / len(losses_mse)
        else:
            loss_mle, loss_l0, loss_mse = sharded_losses[0]
        loss = loss_mle + loss_l0 + loss_mse + tf.losses.get_regularization_loss()
        ...
        if params.validation and params.references[0]:
            ...
            eval_inputs = dataset_c2f_regemb.sort_and_zip_files(files)
            eval_input_fn = dataset_c2f_regemb.get_evaluation_input
        else:
            eval_input_fn = None
