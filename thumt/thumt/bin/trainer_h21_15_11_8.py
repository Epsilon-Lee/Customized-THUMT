...
def override_parameters(params, args):
    ...
    params.vocabulary = {
        ...
        "target_l2": vocabulary.load_vocabulary(params.c2f_vocab[2]),
        "target_l3": vocabulary.load_vocabulary(params.c2f_vocab[3])
    }
...
def main(args):
    ...
    with tf.Graph().as_default():
        if not params.record:
            # Build input queue
            features = dataset.get_training_input_and_c2f_label(params.input, params.c2f_input, params)
        else:
        ...
        losses, losses_l0, losses_l1, losses_l2, losses_l3 = sharded_losses
        loss = tf.add_n(losses) / len(losses)
        loss_l0 = tf.add_n(losses_l0) / len(losses)
        loss_l1 = tf.add_n(losses_l1) / len(losses_l1)
        loss_l2 = tf.add_n(losses_l2) / len(losses_l2)
        loss_l3 = tf.add_n(losses_l3) / len(losses_l3)
        loss = loss + loss_l1 + loss_l2 + loss_l3 + tf.losses.get_regularization_loss()
