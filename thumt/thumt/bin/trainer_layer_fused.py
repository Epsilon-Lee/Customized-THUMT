...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        if len(sharded_losses) > 1:
            fused_losses, losses_l5, losses_l4, losses_l3, losses_l2, losses_l1, losses_l0 = sharded_losses
            fused_loss = tf.add_n(fused_losses) / len(fused_losses)
            loss_l5 = tf.add_n(losses_l5) / len(losses_l5)
            ...
            fused_loss, loss_l5, loss_l4, loss_l3, loss_l2, loss_l1, loss_l0 = sharded_losses[0]
            
        loss = fused_loss + loss_l5 + loss_l4 + loss_l3 + loss_l2 + loss_l1 + loss_l0 + tf.losses.get_regularization_loss()
