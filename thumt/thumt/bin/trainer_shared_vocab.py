...
import thumt.data.dataset_c2f_sv as dataset_c2f_sv
...
def parse_args(args=None):
    ...
    parser.add_argument("--c2f-input", type=str, nargs=1,
                        help="Four paths of target coarse-to-fine label files")
    ...
    parser.add_argument("--c2f-vocabulary", type=str, nargs=1,
                        help="Path of target coarse-to-fine label vocabulary")
    parser.add_argument("--merged-vocabulary", type=str, nargs=1,
                        help="Path of target merged vocabulary")
    parser.add_argument("--private-voc-size", type=int, required=True,
                        help="Path of target merged vocabulary")
...
def override_parameters(params, args):
    ...
    params.merged_vocab = args.merged_vocabulary or params.merged_vocab
    params.private_vocab_size = args.private_voc_size or params.private_vocab_size
    ...
    params.vocabulary = {
        ...
        "target_l0": vocabulary.load_vocabulary(params.c2f_vocab[0]),
        "target_merged": vocabulary.load_vocabulary(params.merged_vocab[0])
    }
    ...
    params.vocabulary["target_l0"] = vocabulary.process_vocabulary(
        params.vocabulary["target_l0"], params
    )
    params.vocabulary["target_merged"] = vocabulary.process_vocabulary(
        params.vocabulary["target_merged"], params
    )
    ...
    params.mapping = {
        ...
        "target_l0": vocabulary.get_control_mapping(
            params.vocabulary["target_l0"],
            control_symbols
        ),
        "target_merged": vocabulary.get_control_mapping(
            params.vocabulary["target_merged"],
            control_symbols
        ),
    }
...
def main(args):
    ...
    with tf.Graph().as_default():
        if not params.record:
            # Build input queue
            features = dataset_c2f_sv.get_training_input_and_c2f_label(params.input, params.c2f_input, params)
        else:
        ...
        if len(sharded_losses) > 1:
            losses_mle, losses_l0 = sharded_losses
            loss_mle = tf.add_n(losses_mle) / len(losses_mle)
            loss_l0 = tf.add_n(losses_l0) / len(losses_l0)
        else:
            loss_mle, loss_l0 = sharded_losses[0]
        loss = loss_mle + loss_l0 + tf.losses.get_regularization_loss()
        ...
        if params.validation and params.references[0]:
            ...
            eval_inputs = dataset_c2f_sv.sort_and_zip_files(files)
            eval_input_fn = dataset_c2f_sv.get_evaluation_input
        else:
            eval_input_fn = None
