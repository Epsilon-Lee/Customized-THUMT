        ...
        for layer in range(params.num_decoder_layers):
            ...
            with tf.variable_scope(layer_name):
                ...
                with tf.variable_scope("feed_forward"):
                    ...
                    x = _residual_fn(x, y, 1.0 - params.residual_dropout)
                    x = _layer_process(x, params.layer_postprocess)
            layer_wise_state[layer_name] = y  # x, y can both be tried
...
def decoding_graph_l4_expect(features, state, mode, params):
    ...
    t_merged_vocab = params.vocabulary["target_merged"]
    tgt_merged_vocab_size = len(t_merged_vocab)
    private_vz = params.private_vocab_size
    fine_indices = tf.constant(list(range(tgt_vocab_size)))
    coarse_indices = list(range(private_vz)) + list(range(tgt_vocab_size, tgt_merged_vocab_size))
    coarse_indices = tf.constant(coarse_indices)
    ...
    if params.shared_source_target_embedding:
        ...
        tgt_embedding = tf.get_variable("target_merged_embedding",
                                        [tgt_vocab_size, hidden_size],
                                        initializer=initializer)
    ...
    if params.shared_embedding_and_softmax_weights:
        ...
        merged_weights = tf.get_variable("merged_softmax", [tgt_merged_vocab_size, hidden_size],
                                  initializer=initializer)
        weights = tf.gather(merged_weights, fine_indices)
        weights_l4 = tf.gather(merged_weights, coarse_indices)
