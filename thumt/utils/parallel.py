...
def shard_features(features, device_list, constant_features=None):
    ...
    for d in range(num_datashards):
        ...
        if constant_features:
            feat.update(constant_features)
        datashard_to_features.append(feat)

    return datashard_to_features


def parallel_model(model_fn, features, devices, use_cpu=False, constant_features=None):
    ...
    if len(devices) == 1:
        if constant_features:
            features.update(constant_features)
        return [model_fn(features)]

    features = shard_features(features, devices, constant_features)
